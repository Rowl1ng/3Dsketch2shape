...
class Trainer(object):
    ...
    def init_network(self):
        ...
        # if self.config.load_Pinaki_model:
    ...
    def load_model(self, last_resume_ckpt, load_optimizer=True, load_NF=True):
        ...
        if hasattr(self, "latent_flow_network") and load_optimizer and load_NF:
    ...
    def eval_encoder_epoch(self, epoch, data_loader):
        ...
        for pc_data, voxel_data, sdf_data, indices in data_loader:
...
if __name__ == '__main__':
    ...
    optional_args = [("mode", str, "eval"),
                    ("run_id", int, 0), ("resume_path", str, 'configs/flow_sdf/recon_L1+NCE+sketch_SDF_scale1.py'),  ("batch_size", int, 6),
    ...
    if not hasattr(config, 'load_Pinaki_model') :
        config.load_Pinaki_model = False
    ...
    if hasattr(config, 'disentangle'):
        from disentangled_flow_sdf_trainer import Disentangle_Trainer
    ...
    def inference():
        ...
        selected = [143, 61, 11, 129, 107]# np.random.choice(len(trainer.test_samples), args.num_samples, replace=False)
